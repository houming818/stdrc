...
prompt_end() {
	...
	declare -a codes=($(text_effect reset) $(fg_color red))
...
prompt_context() {
    ...
    if [[ "$user" != "root" ]]; then
        prompt_segment default cyan "$(date +%H:%M:%S)"
...
prompt_git() {
	...
	PR="$PR${ref/refs\/heads\//git@}$dirty"
    fi
}

# Dir: current working directory
prompt_dir() {
    prompt_segment default white '$(pwd)'
}
...
prompt_status() {
    ...
    #[[ $UID -eq 0 ]] && symbols+="$(ansi_single $(fg_color red))ROOT"
    [[ $(jobs -l | wc -l) -gt 0 ]] && symbols+="$(ansi_single $(fg_color green)) J$(jobs -l | wc -l)"
